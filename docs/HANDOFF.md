...
- **WSL repo (edit + commit here):** `/home/natkins/n8builds/tools/t-hub/t-hub-app`
  (the repo is `t-hub-app` inside the `tools/t-hub` umbrella, alongside
  `t-hub-videos`; the repo *is* the t-hub monorepo). `main` is integration.
...
cd /home/natkins/n8builds/tools/t-hub/t-hub-app && git add -A && git commit ...   # ASCII-only msg + Co-Authored-By trailer
