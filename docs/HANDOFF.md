...
> All terminal IDs, Captain records, installed versions, commit references,
> endpoints, credentials, worktrees, and run instructions in this document are
> historical snapshots.
> Do not use them as current control endpoints, authorization, release
> provenance, or resume points.
> The current source baseline is tracked by Git, while current packaged proof is
> governed by `docs/PACKAGE-5-LIVE-ACCEPTANCE.md` and
> `docs/PACKAGE-6-PERFORMANCE-CLOSURE.md`.
...
**MERGED TODAY (all general's word):** #61-#66 (prior context) + #68 CI gate (+ branch protection now REQUIRES Rust/Frontend checks on main - the audit CRITICAL is closed), #69 (GH_CONFIG_DIR/startupCommand/readopt), #70 (pane-scrape matrix proof; the audit HIGH was stale - #62 already enforced), #71 (MCP stale-pin + bridge-heal env-token preservation).
**IMMEDIATE TAIL (finish these):**
1. **#72 delta** - reviewer-B 7e9c3970 verifying the P72-1/P72-2 fixes (sentinel `delta-72.done`) -> merge #72 on clean verdict (general's standing "merge what is mergeable" covers reviewed-ready+CI-green, stated with opt-out) -> reap f7fc6b22 + 7e9c3970.
2. **#67+#73 verify-install** - crew 8dfa5c8a building a TEST NSIS from `captain-first-class` (= #67 tip + #73; sentinel `build-verify.done`; NO bump, do not auto-install). Then the GENERAL's ~2-min click-test: captain stops app, silent-installs, relaunches; general clicks Create Orchestrator (create/no-duplicate/restart-Restore) + a control-toggle spawn. PASS -> merge #67 -> reap 5547da43 + 8c130396 -> rebase #73 onto main, retarget base, undraft, CI green -> merge #73 -> reap 8dfa5c8a.
3. **Batched 0.3.65 deploy** (general's confirm): one bump+build+install off main ships #66 Board tab + #68-#73 together; post-install checks = board tab renders, control-toggle spawn mints control, audit_verify answers, MCP no longer strands read-only after restart.
**THEN - POWDER INTEGRATION (general-confirmed: powder rides this change wave; ack sent to powder captain c7f3135a):** (a) #66 ships in the 0.3.65 deploy - no extra install; (b) POWDER-2: claim agent = bare t-hub session id -> resolve-to-live-tile + focus from Board card click (small crew, spawn as wave crews reap); (c) POWDER-3 event bridge vs LOCAL powder :4000 (SSE tail first; wakes the owning captain, RETIRES the /tmp sentinel-watcher pattern); prod cutover gated on powder Phase-0 (they hand tailnet URL + agent-scope key; flip powderBoardUrl default); (d) POWDER-4 feed = design-doc first, general reviews before any code. Old staged briefs: `/tmp/flap-probe/POWDER-1-WEBVIEW-BRIEF.md` (shipped), powder items packet `~/.t-hub/captain/reviews/powder-thub-integration-request-2026-07-11.md`.
**BACKLOG (audit master, next waves):** god-file refactor (control.rs/workspace.ts), P72-3/4 follow-ups (disk monitoring for fail-closed), #67 L2/L4/L5 LOWs, webview placement-drop bug (general saw it live again today - socket-spawned tiles invisible until window reload; folded as first-class into the tile-attach/adopt bundle, spawn a dedicated crew if it reproduces post-reload), Codex Phase-1 PR-B/PR-C (plan `codex-phase1-plan-2026-07-11.md`; PR-A/#63 merged), orchestrator-representation items.
**Fleet notes:** appturnity captain re-keyed to 4e714030 (control, bypass, resumed) - NOT mine; Cortana f1293f10 control-capable; roster + sentinels in `~/.t-hub/captain/ships/t-hub-scribe.md` (sentinel dir `/tmp/t-hub-crew-done/t-hub-scribe/`, watcher pattern in log).
...
**BUILD QUEUE (unchanged discipline - worktree crew Opus 4.8 high permissionless, PR-only, NO bump, xhigh review, fix round, same-reviewer delta, the general's word):** item-1 Phase-3 ACLs (NEXT; consumes item-3's session-token-on-ControlRequest + plane-confirm seams, documented in the PR #59 body) -> **tile-attach fix (general-queued 2026-07-11):** background tiles must never attach as ~2-column clients + reassert `window-size latest` on attach - retires the manual-220x50 crew-pane doctrine and with it BOTH the fill-character dot fields AND the right-edge clipping (root cause: `reviews/dots-artifact-finding-2026-07-11.md`; interim mask: Cortana set `fill-character` blank on the socket) -> item-4 build -> item-1 Phase-4 typing-guard LAST.
**BUILDS ON HOLD (general's pacing, 2026-07-11 ~13:4x):** nothing spawns until Cortana sequences the fleet capability-cycle (control-capable re-spawns + `claude --resume` migrations); the captain's read tier makes every spawn need a direct general confirm until then.
