...
1. **Codex Phase-1 - VERDICT: PLAN-READY, general pre-authorized implementation on pass ("if that passes then implement")**. Plan: `~/.t-hub/captain/reviews/codex-phase1-plan-2026-07-11.md`. Shape: 3 crew PRs - PR-A adapter seam + spawn presets + codex-mcp-add provisioning + resume wiring (M+3S); PR-B `codex exec --json | t-hub-agent --codex-tap` lifecycle producer (M); PR-C Codex continuity catalog (M). ZERO contested files (parallel-safe with Phase-3 ACLs - verified). Codex 0.142.5 installed, all flags on-binary-verified. Event map test-locked (thread.started->SessionStart, turn.completed->Stop; SessionEnd SUPPRESSED on clean exit - naive mapping evicts sessions). 1 HIGH folded (prose-steer into a codex crew shell executes as commands - brief-doctrine fix), 6 MED + 5 LOW folded. TASK 0 = record 0.142.5 fixtures. Full gate per PR (PR-only, no bump, xhigh review, fix round, delta, general's merge word). Background: `codex-harness-audit-2026-07-11.md`.
