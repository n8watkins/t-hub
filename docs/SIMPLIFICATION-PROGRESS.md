...
`control.rs` has gone from 73,435 to ~12,849 lines total across this effort.
...
**Remaining WS1 work that is IN the original plan** - finish `captains_registry.rs` (the data model), IN PROGRESS:
The plan scoped this module at ~7,600 lines ("the `impl CaptainsRegistry` state machine + `FleetRole`/`ClaimState` enums"); the ~5,500-line inherent impl + test/Default impls are moved, and now the first model sub-cluster too. It is a NON-CONTIGUOUS, multi-sub-cluster move (the model types interleave with the wire types that stay, the deferred Powder Phase-B types (WS4), and business-logic free fns).
Model sub-clusters and status:
- DONE (`0f267a4`): `FleetRole` (+Default/label), `ClaimState`, `CrewState`, `CrewRef` (+`new`). All `pub`, re-exported via `pub use captains_registry::{...}` in `control.rs` (needed for `control::FleetRole` from `acl.rs`); `captains_registry`'s own `use super::*;` re-imports them harmlessly (local defs shadow).
- REMAINING (biggest/hairiest LAST): `ProjectRecord`; `FleetIdentity`/`CaptainRecord` alias; `SnapshotReadError`/`ClaimDisposition`/`ClaimOutcome`/`ShipMembership`; `FleetWorkspaceOwner`/`FleetWorkspaceRecord`/`PendingFleetOperation*`/`CaptainsSnapshot`; and the tail machinery cluster (authority projections, `AuthorityGenerationChanges`/`AuthorityGenerations`, `CaptainsInner`, `AgentDeliveryUpdate`, the `CaptainsRegistry` STRUCT, `CrewPowderOperationGuard`).
- KEY DIFFICULTY for the tail cluster: it mixes TRAIT impls (`From`/`Default`/`Drop` - methods take NO visibility modifier) with INHERENT impls (methods need `pub(super)`), so a blanket `^    fn ` -> `pub(super)` transform is WRONG there - handle inherent vs trait impls separately (per-impl), like the RequestCache-vs-Default split in `idempotency.rs`. Private types -> `pub(super)` (add a private `use captains_registry::*;` in `control.rs` for the bare names); the `CaptainsRegistry` struct is `pub` -> add it to the `pub use`. Watch struct FIELDS the parent/tests inspect (make those `pub(super)` too).
- Leave the Powder Phase-B types (`PowderWorkBinding`/`PendingDispatch*`/`PowderMutation*`/`PowderProjectBinding`/`PowderWorkState`) and shared helpers (`now_ms`, schema-version consts) where they are.
...
Done (type clusters): folded `#[cfg(test)]` + `Default` `CaptainsRegistry` impls into `captains_registry.rs`; `idempotency.rs` (`RequestCache` + `CaptainControlLeases`); captains data-model pt.1 (`FleetRole`/`ClaimState`/`CrewState`/`CrewRef`) into `captains_registry.rs`.
