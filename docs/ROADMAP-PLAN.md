...
- **Scope:** new `store/keybindings.ts` (prefix + bindings, persisted), `lib/keymapExecutor.ts` (command registry — migrate all current hotkeys into it), `lib/prefixKeyHandler.ts` (armed-state machine + visual hint + timeout), `components/CommandPalette.tsx` (fuzzy + interactive rebind), Settings keybindings section. Refactor `Canvas.tsx` keydown to dispatch through the registry.
- **Effort:** ~1 wk (~5 new files, ~1000 LOC).
- **Risks:** **`Ctrl+A` collides with shell readline (beginning-of-line)** — the classic prefix tradeoff. Mitigate: configurable prefix + "press prefix twice → send literal." Validate persisted bindings on load (fall back to defaults). Don't shadow browser/Tauri devtools keys.
- **Decision needed:** default prefix — `Ctrl+A` (tmux-screen muscle memory, readline collision) vs a collision-free combo like `Ctrl+Space`. *Recommend configurable, default `Ctrl+A`, with double-tap-to-send-literal.*
- **Accept:** `prefix → c` = new tab; palette opens/fuzzes/executes; rebinding persists; every existing hotkey still works.
