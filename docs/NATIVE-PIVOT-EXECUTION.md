...
- 2026-07-02 T14a DONE (branch `t14-audit`): the parity AUDIT half of T14 - matrix + distribution story + small-gap closure. The cutover flip itself (steps in the doc's §5) stays with the general.
  - **Deliverable: [T14-PARITY.md](./T14-PARITY.md)** - 61-row matrix (webview `apps/desktop/src` audited as the spec vs `apps/native/src`): **25 present / 8 degraded / 24 missing / 4 wontport**, each row with effort + owner. Gap concentration: 8 missing rows sit with the in-flight chrome crews; 5 fold into one keymap/palette/action-registry task (T-A, the biggest daily-drive gap); 6 into a flow-gaps task (T-B: local spawn affordance, resume wiring incl. the server-side `spawn_terminal` startup-command arg, native-only `remove_worktree`, worktree UX, chimes/OS-notify, single-instance); 5 are post-flip polish (T-C). WSL-boundary checks all hold (doc §2).
