...
| **0.3.14** | `e82b49c` | **Cold-first-drag fix "Option A": suppress focus-triggered work during a window drag** (`windowInteraction.ts` `runWhenIdle` + `isInteracting`; wrap all 6 focus handlers) **+ gate Codex polling on an open Codex tile** | ✅ **user-verified: deterministic cold-drag delay GONE ("no delay on drag")**. ⚠️ Residual SPORADIC unresponsiveness remains — that's the separate hung-window item below, not the focus storm. |
