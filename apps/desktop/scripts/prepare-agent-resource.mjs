...
function buildOnWindows() {
  ...
  const buildScript =
    'set -eu; unset CARGO_HOME RUSTUP_HOME RUSTC RUSTDOC; cd "$1"; RUSTC_WRAPPER= CARGO_BUILD_RUSTC_WRAPPER= CARGO_TARGET_DIR="$2" cargo build --locked --release -p t-hub-agent';
  run("wsl.exe", [
    "-e",
    "bash",
    "-lc",
