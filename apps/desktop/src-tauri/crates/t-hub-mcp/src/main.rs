//! `t-hub-mcp` — T-Hub's local MCP server (PRD §9.6, §11.2).
//!
...
//! `tools/call` to the running T-Hub app over a **loopback control channel**
//! (the app listens on `127.0.0.1:<port>`; this binary discovers the port +
//! per-launch token from `~/.t-hub/control.json`). Because forwarding is by
    ...
    match mode {
        Mode::Serve => {
            ...
            if let Err(e) = server::run(reader, writer) {
                eprintln!("t-hub-mcp: server loop exited with error: {e}");
        ...
        Mode::Version => {
            println!("t-hub-mcp {}", env!("CARGO_PKG_VERSION"));
        }
    }
}
...
fn parse_args() -> Mode {
    for arg in std::env::args().skip(1) {
        match arg.as_str() {
            ...
            other => {
                eprintln!("t-hub-mcp: ignoring unknown argument {other:?}");
            }
        }
    }
    Mode::Serve
