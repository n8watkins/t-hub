...
/// The bearer is the singleton T-Hub itself recorded: this exact identity, in this
/// exact terminal, per `captains.json`.
///
/// Weaker than [`authoritative_cortana_identity`], which additionally requires the
/// published Fleet claim. This is the PRE-CLAIM state - a shell T-Hub created and
/// seeded with the Cortana identity, before the user has started an agent in it
/// that introduces itself. It authorizes exactly two things: reading the bootstrap
/// snapshot, and claiming the Cortana role for this same terminal.
fn recorded_cortana_singleton(ctx: &ControlContext, identity_id: &str, terminal_id: &str) -> bool {
    let durable = ctx.captains.cortana_identity();
    durable.identity_id.as_deref() == Some(identity_id)
        && durable.terminal_id.as_deref() == Some(terminal_id)
        && ctx
            .identity
            .get(identity_id)
...
fn authoritative_cortana_identity(
    ...
    if durable.identity_id.as_deref() != Some(identity.id.as_str())
    ...
    // The Fleet claim is what remains of the old attestation: a Cortana SHELL is
    // created by T-Hub and holds the identity, but authority is only published once
    // an agent running in it has claimed the singleton (`claim_captain`, role
    // cortana). An empty shell therefore carries no control authority - which is
    // the intended posture, since nothing is running in it to exercise any.
...
fn enforce_attach_authority(
    ...
    if role == FleetRole::Cortana {
        // The recorded singleton may claim ITSELF. Nothing auto-starts an agent in
        // the Cortana shell any more, so the agent the user starts there is what
        // publishes the Fleet claim - and before that claim exists it is not yet
        // "authoritative Cortana", which would make this an unreachable role.
        //
        // The authorization is the durable record: T-Hub created this exact shell
        // and injected this exact identity into it. It is deliberately narrower
        // than apex - only this terminal, only this bearer.
        if caller.is_some_and(|caller| {
            caller.tile.as_deref() == Some(target_terminal)
                && recorded_cortana_singleton(ctx, &caller.session_id, target_terminal)
        }) {
            return Ok(());
        }
...
fn authorize_cortana_bootstrap(
    ...
    // The durable record IS the authorization: T-Hub created (or reattached) this
    // exact shell and injected this exact identity into it. This is the bootstrap
    // path an agent takes BEFORE it claims the Fleet role, so it deliberately does
    // not require a claim.
    if !recorded_cortana_singleton(ctx, &identity.id, &terminal_id) {
        return Err(
            "cortana_bootstrap: authenticated bearer is not the recorded Cortana singleton".into(),
        );
    }
    // Once a claim exists it must be exactly this terminal's. An ambiguous or
    // foreign claim means two things believe they are Cortana; refuse rather than
    // hand out a fleet snapshot.
    let claimed = ctx
        .captains
        .snapshot()
        .captains
    ...
    match claimed.as_slice() {
        [] => {}
        [claim] if claim.terminal_id.as_deref() == Some(terminal_id.as_str()) => {}
        _ => return Err("cortana_bootstrap: Fleet claim is missing or ambiguous".into()),
    }
