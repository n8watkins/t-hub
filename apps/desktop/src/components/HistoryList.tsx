...
const TERMINAL_SESSION_STATUSES = new Set<SessionStatusEvent["status"]>([
  "failed",
  "detached",
  "expired",
  "unknown",
...
export function HistoryList({ onCount }: HistoryListProps) {
  ...
  useEffect(() => {
    ...
    const offWorkspace = useWorkspace.subscribe((state) => {
...
function HistoryRow({
  ...
  const act = useCallback(async () => {
    ...
    try {
      ...
      // Preserve the request ID only when the response leg is ambiguous. An
      // authoritative refusal gets a fresh operation identity on the next click.
      if (!isAmbiguousHistoryFailure(reason)) requestId.current = null;
