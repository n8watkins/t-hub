// The settings/theme panel - the heavy half of the old ThemeEditor.tsx.
//
// SPLIT OUT FOR STARTUP COST: this tier statically pulls in RecoveryReview,
// HookInstallPanel, CodexHookInstallPanel, VoiceSection, ThemeSection and the
// updater/process plugins. ThemeProvider mounts from an index.html entry, so all
// of that used to sit in the BOOT graph and be parsed on every launch for a panel
// that only opens on Ctrl/Cmd+,. ThemeEditor now React.lazy()s this module, so it
// is fetched on first open instead.
//
// `ThemeEditorPanel` is the default export (what React.lazy resolves).
...
// Theme section (presets + colors/layout/typography/terminal tabs) lives in its
// own file so this module stays focused on the App-behavior sections + the panel
// shell. See theme-editor/ThemeSection.tsx.
import { ThemeSection } from "./theme-editor/ThemeSection";
 ...
 * which aren't chrome tokens). Same visual style as the chrome-token ToggleRow,
 * with an optional muted helper line under the label.
 *
 * Like that ToggleRow, the row is a plain <div> and the label is plain text:
...
// The theme-tab icons (save/export/import/reset/chevron) + the compact IconBtn
// they sit in live in theme-editor/; only the modal-header CloseIcon is here.
// ---------------------------------------------------------------------------
