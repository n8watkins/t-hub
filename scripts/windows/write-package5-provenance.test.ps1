...
try {
  ...
  Assert-Fails { & $scriptPath -OutputPath (Join-Path ([System.IO.Path]::GetTempPath()) "non-rfc-installed-at.json") -RepositoryRoot $root -SourceCommit $head -InstallerPath $installer -RawBinaryPath $raw -ExpectedBinaryPath $expected -ExtractedBinaryPath $extracted -InstalledBinaryPath $installed -InstalledAt "07/23/2026 12:00:00" -ValidatorPath $validator -ValidationPath $validator } "RFC3339"
